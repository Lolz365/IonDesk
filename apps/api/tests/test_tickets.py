...
@pytest.mark.anyio
async def test_ticket_list_rejects_duplicate_status_query_parameters(
    api_client: tuple[AsyncClient, Callable[[TenantContext], None]],
) -> None:
    client, set_context = api_client
    set_context(
    ...
    response = await client.get("/api/v1/tickets?status=new&status=assigned")
...
@pytest.mark.anyio
async def test_ticket_creation_rejects_title_containing_newline_without_write(
    api_client: tuple[AsyncClient, Callable[[TenantContext], None]],
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    async with session_factory() as session, session.begin():
        organization = Organization(
            identifier="newline-title-org", name="Newline Title Org"
        )
        owner = User(
            oidc_subject="newline-title-owner",
            email="newline-title-owner@example.test",
            display_name="Newline Title Owner",
        )
        session.add_all([organization, owner])
        await session.flush()
        membership = Membership(
...
@pytest.mark.anyio
async def test_ticket_status_transition_rejects_idempotency_key_containing_whitespace(
    api_client: tuple[AsyncClient, Callable[[TenantContext], None]],
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    async with session_factory() as session, session.begin():
        organization = Organization(
            identifier="whitespace-status-key-org",
            name="Whitespace Status Key Org",
        )
        dispatcher = User(
            oidc_subject="whitespace-status-key-dispatcher",
            email="whitespace-status-key-dispatcher@example.test",
            display_name="Whitespace Status Key Dispatcher",
        )
        session.add_all([organization, dispatcher])
        await session.flush()
        membership = Membership(
        ...
        ticket = Ticket(
            organization_id=organization.id,
            created_by_user_id=dispatcher.id,
            title="Do not assign with an invalid key",
        )
        session.add_all([membership, ticket])
        await session.flush()
        context = TenantContext(
    ...
    response = await client.patch(
        f"/api/v1/tickets/{ticket_id}/status",
        headers={"Idempotency-Key": "status transition 1"},
        json={"status": "assigned"},
    )
