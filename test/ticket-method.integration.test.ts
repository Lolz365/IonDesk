...
test("DELETE /api/tickets reports the methods allowed for the known route", async () => {
  ...
  try {
    ...
    assert.deepEqual(await response.json(), {
      error: {
        ...
        message: "Method DELETE is not allowed for /api/tickets",
      },
    });
  } finally {
...
test("DELETE /api/tickets/:id/resolve reports POST for the known nested route", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-ticket-method-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
    const response = await fetch(`${server.baseUrl}/api/tickets/missing/resolve`, {
      method: "DELETE",
    });
    ...
    assert.deepEqual(await response.json(), {
      error: {
        code: "method_not_allowed",
        message: "Method DELETE is not allowed for /api/tickets/:id/resolve",
      },
    });
  } finally {
    if (server) await stopServer(server.process);
