...
test("POST / reports GET for the known root route", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-root-method-"));
  ...
  try {
    ...
    assert.deepEqual(await response.json(), {
      error: {
        ...
        message: "Method POST is not allowed for /",
      },
    });
  } finally {
