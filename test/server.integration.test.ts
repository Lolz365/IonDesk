...
test("rejects a non-string title when creating a ticket", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-server-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
...
test("rejects a non-string description when creating a ticket", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-server-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
    const response = await fetch(`${server.baseUrl}/api/tickets`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ title: "Inspect pipe", description: 42 }),
    });
...
test("rejects unsupported ticket list query parameters", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-server-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
...
test("rejects unsupported fields when analyzing a ticket photo", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-server-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
    ...
    const response = await fetch(`${server.baseUrl}/api/tickets/${ticket.id}/photos/${photo.id}/analyze`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ signals: [], status: "ready" }),
    });
    ...
    assert.deepEqual(await response.json(), {
      error: {
        code: "validation_error",
        message: "Unsupported analysis field: status",
      },
    });
  } finally {
    if (server) await stopServer(server.process);
...
test("rejects unsupported fields in ticket photo analysis signals", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "visualops-server-"));
  let server: RunningServer | undefined;
  try {
    server = await startServer(dataDir);
    ...
    const response = await fetch(`${server.baseUrl}/api/tickets/${ticket.id}/photos/${photo.id}/analyze`, {
      ...
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        signals: [{ label: "corrosion", confidence: 0.9, status: "ready" }],
      }),
    });
    ...
    assert.deepEqual(await response.json(), {
      error: {
        code: "validation_error",
        message: "Unsupported signals[0] field: status",
      },
    });
  } finally {
    if (server) await stopServer(server.process);
